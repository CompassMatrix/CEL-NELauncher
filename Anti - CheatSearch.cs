...
namespace NeteaseLogin.NeteaseLogin.Memory
{
    class WriteMethod
    {
        ...
        internal class ProcessModules
        {
            public unsafe static ProcessModule64[] getWOW64Modules(int PID)
            {
                ...
                {
                    Debug.Write("获取LDTE64结构失败,如果一直出现可能无法获取进程模块");
                    DLLTool.CloseHandle(num);
                    return new ProcessModule64[0];
                }
        ...
        public static int x64_写内存字节集(int processId, ulong address, byte[] buffer, ulong size)
        {
            int num = DLLTool.OpenProcess(2035711, 0, processId);
            ulong nReturnSize;
            int result = DLLTool.ZwWow64WriteVirtualMemory64(num, address, buffer, size, out nReturnSize);
            DLLTool.CloseHandle(num);
            return result;
        }
    }
